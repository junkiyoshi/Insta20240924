...
void ofApp::draw() {
	...
	for (int k = 0; k < this->param_list.size(); k++) {
		...
		ofPushMatrix();
		ofTranslate((k % 3) * 220 - 220, (k / 3) * 220 - 220);
		...
		for (int i = 0; i < len; i++) {
			...
			auto noise_radius = ofMap(ofNoise(this->param_list[k].y, (ofGetFrameNum() + i) * 0.01), 0, 1, -100, 100);
			auto next_noise_deg = ofMap(ofNoise(this->param_list[k].x, (ofGetFrameNum() + i + 1) * 0.0065), 0, 1, -360, 360);
			auto next_noise_radius = ofMap(ofNoise(this->param_list[k].y, (ofGetFrameNum() + i + 1) * 0.01), 0, 1, -100, 100);
		...
		ofColor color;
		int span = ofMap(ofNoise(this->param_list[k].z), 0, 1, 0, 18);
